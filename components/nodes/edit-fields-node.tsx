...
import { PenLine, Trash2, CheckCircle2, XCircle, Loader2, AlertCircle, Settings2, Play } from "lucide-react";
...
export const EditFieldsNode = memo(({ data, selected, id }: NodeProps<EditFieldsNodeData>) => {
  ...
  const removeCount = data.config?.removeFields?.length || 0;
    ...
    <NodeWrapper nodeId={id} hasBreakpoint={data.hasBreakpoint} isBreakpointActive={data.isBreakpointActive} onToggleBreakpoint={data.onToggleBreakpoint}>
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="font-bold text-sm">{data.label || "Edit Fields"}</div>
            <div className="text-xs text-muted-foreground">
              {fieldCount > 0 || removeCount > 0
                ? `${fieldCount} set, ${removeCount} remove`
                : 'Set/remove fields'}
            </div>
          </div>
          {getStatusIcon()}
          ...
          <div className="flex-1 overflow-auto">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{data.label || "Edit Fields"}</p>
              </div>
              <div>
