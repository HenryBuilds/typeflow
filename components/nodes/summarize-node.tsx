...
import { Calculator, Trash2, CheckCircle2, XCircle, Loader2, AlertCircle, Settings2, Play } from "lucide-react";
...
export const SummarizeNode = memo(({ data, selected, id }: NodeProps<SummarizeNodeData>) => {
  ...
  const operationCount = data.config?.operations?.length || 0;
    ...
    <NodeWrapper nodeId={id} hasBreakpoint={data.hasBreakpoint} isBreakpointActive={data.isBreakpointActive} onToggleBreakpoint={data.onToggleBreakpoint}>
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="font-bold text-sm">{data.label || "Summarize"}</div>
            <div className="text-xs text-muted-foreground">
              {operationCount > 0 ? `${operationCount} operation${operationCount > 1 ? 's' : ''}` : 'Sum, count, etc.'}
            </div>
          </div>
          {getStatusIcon()}
        </div>
          ...
          <div className="flex-1 overflow-auto">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{data.label || "Summarize"}</p>
              </div>
              <div>
