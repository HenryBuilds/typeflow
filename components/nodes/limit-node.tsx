...
import { ArrowDown, Trash2, CheckCircle2, XCircle, Loader2, AlertCircle, Settings2, Play } from "lucide-react";
...
interface LimitNodeData {
  label?: string;
  config?: {
    maxItems?: number;
    keepFirst?: boolean; // true = keep first N, false = keep last N
  };
...
export const LimitNode = memo(({ data, selected, id }: NodeProps<LimitNodeData>) => {
  ...
  const maxItems = data.config?.maxItems || 10;
    ...
    <NodeWrapper nodeId={id} hasBreakpoint={data.hasBreakpoint} isBreakpointActive={data.isBreakpointActive} onToggleBreakpoint={data.onToggleBreakpoint}>
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="font-bold text-sm">{data.label || "Limit"}</div>
          ...
          <div className="flex-1 overflow-auto">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{data.label || "Limit"}</p>
              </div>
              <div>
