...
import { SplitSquareVertical, Trash2, CheckCircle2, XCircle, Loader2, AlertCircle, Settings2, Play } from "lucide-react";
...
interface SplitOutNodeData {
  label?: string;
  config?: {
    fieldToSplit?: string; // The array field to split
    includeOtherFields?: boolean; // Include other fields in output
  };
...
export const SplitOutNode = memo(({ data, selected, id }: NodeProps<SplitOutNodeData>) => {
    ...
    <NodeWrapper nodeId={id} hasBreakpoint={data.hasBreakpoint} isBreakpointActive={data.isBreakpointActive} onToggleBreakpoint={data.onToggleBreakpoint}>
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="font-bold text-sm">{data.label || "Split Out"}</div>
            <div className="text-xs text-muted-foreground">
              {data.config?.fieldToSplit ? `Split: ${data.config.fieldToSplit}` : 'Configure field'}
            </div>
          </div>
          {getStatusIcon()}
        </div>
          ...
          <div className="flex-1 overflow-auto">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{data.label || "Split Out"}</p>
              </div>
              <div>
