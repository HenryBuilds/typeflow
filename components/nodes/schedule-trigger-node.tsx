...
export const ScheduleTriggerNode = memo(({ data, selected, id }: NodeProps<ScheduleTriggerNodeData>) => {
  ...
  const getScheduleDescription = () => {
    if (data.config?.mode === "cron" && data.config?.cronExpression) {
      return data.config.cronExpression;
    }
    const interval = data.config?.interval || 1;
    const unit = data.config?.intervalUnit || "hours";
    ...
    <NodeWrapper nodeId={id} hasBreakpoint={data.hasBreakpoint} isBreakpointActive={data.isBreakpointActive} onToggleBreakpoint={data.onToggleBreakpoint}>
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="font-bold text-sm">{data.label || "Schedule"}</div>
          ...
          <div className="flex-1 overflow-auto">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{data.label || "Schedule"}</p>
              </div>
              <div>
