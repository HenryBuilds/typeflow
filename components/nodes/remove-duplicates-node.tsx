...
import { Copy, Trash2, CheckCircle2, XCircle, Loader2, AlertCircle, Settings2, Play } from "lucide-react";
...
interface RemoveDuplicatesNodeData {
  label?: string;
  config?: {
...
export const RemoveDuplicatesNode = memo(({ data, selected, id }: NodeProps<RemoveDuplicatesNodeData>) => {
    ...
    <NodeWrapper nodeId={id} hasBreakpoint={data.hasBreakpoint} isBreakpointActive={data.isBreakpointActive} onToggleBreakpoint={data.onToggleBreakpoint}>
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="font-bold text-sm">{data.label || "Remove Duplicates"}</div>
            <div className="text-xs text-muted-foreground">
              {fieldCount > 0 ? `By ${fieldCount} field${fieldCount > 1 ? 's' : ''}` : 'All fields'}
            </div>
          </div>
          {getStatusIcon()}
        </div>
          ...
          <div className="flex-1 overflow-auto">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{data.label || "Remove Duplicates"}</p>
              </div>
              <div>
...
RemoveDuplicatesNode.displayName = "RemoveDuplicatesNode";
