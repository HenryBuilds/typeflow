...
import { AlertTriangle, Trash2, CheckCircle2, XCircle, Loader2, AlertCircle, Play, Settings2 } from "lucide-react";
...
export const ThrowErrorNode = memo(({ data, selected, id }: NodeProps<ThrowErrorNodeData>) => {
  ...
  const configuredMessage = data.config?.errorMessage || "Error";
    ...
    <NodeWrapper nodeId={id} hasBreakpoint={data.hasBreakpoint} isBreakpointActive={data.isBreakpointActive} onToggleBreakpoint={data.onToggleBreakpoint}>
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="font-bold text-sm">{data.label || "Throw Error"}</div>
      ...
      {/* No source handle - this node terminates the workflow */}
          ...
          <div className="flex-1 overflow-auto">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{data.label || "Throw Error"}</p>
              </div>
              <div>
...
ThrowErrorNode.displayName = "ThrowErrorNode";
