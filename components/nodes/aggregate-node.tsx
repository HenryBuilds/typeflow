...
import { ListPlus, Trash2, CheckCircle2, XCircle, Loader2, AlertCircle, Settings2, Play } from "lucide-react";
...
interface AggregateNodeData {
  label?: string;
  config?: {
    fieldToAggregate?: string; // Field to collect into array
    outputFieldName?: string; // Name of the output array field
  };
...
export const AggregateNode = memo(({ data, selected, id }: NodeProps<AggregateNodeData>) => {
    ...
    <NodeWrapper nodeId={id} hasBreakpoint={data.hasBreakpoint} isBreakpointActive={data.isBreakpointActive} onToggleBreakpoint={data.onToggleBreakpoint}>
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="font-bold text-sm">{data.label || "Aggregate"}</div>
            <div className="text-xs text-muted-foreground">
              {data.config?.fieldToAggregate ? `Field: ${data.config.fieldToAggregate}` : 'Combine items'}
            </div>
          </div>
          {getStatusIcon()}
        </div>
          ...
          <div className="flex-1 overflow-auto">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">{data.label || "Aggregate"}</p>
              </div>
              <div>
